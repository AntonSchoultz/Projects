...
public class XTag
{
  ...
  public boolean isEmpty()
  {
    if ( name != null && name.trim().length() > 0 )
      return false;
    if ( body != null && body.replace('\r', ' ').replace('\n', ' ').trim().length() > 0 )
      return false;
    return true;
  }
  ...
  /**
   * Return a string representation of this tag and it's children (returns XML string)
   */
  ...
  /**
   * Return a string representation of this tag and it's children (returns XML string using the
   * provided tab to format the string)
   */
  public String toXmlString(String tab)
  {
    ...
    if ( children.size() > 0 )
    {
      ...
      if ( !kid.getName().equalsIgnoreCase("") && !kid.getName().equalsIgnoreCase("[") )
      {
        sb.append(tab);
      }
      sb.append("</");
  ...
  private void appendAttributes(String tab, StringBuffer sb)
  {
    ...
    if ( attributes.size() > 0 )
    {
      for (String attribName : attributes.keySet())
      {
        ...
        if ( attrVal != null )
        {
          if ( ((sb.length() - bx) + attribName.length() + attrVal.length()) > WRAP_LEN )
          {
  ...
  /**
   * Returns the first child which has the requested tag name, null if not found
   */
